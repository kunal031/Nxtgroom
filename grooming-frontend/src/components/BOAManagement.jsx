...
export default function BOAManagement() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      console.error("Failed to create BOA", err);
      alert("Failed to create BOA");
    }
  };
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr>
                  <td colSpan="4" className="p-8 text-center text-slate-400">Loading BOAs...</td>
                </tr>
              ) : boas.length === 0 ? (
                <tr>
                  <td colSpan="4" className="p-8 text-center text-slate-400">No BOAs found.</td>
                </tr>
              ) : (
                boas.map(boa => (
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-xl font-extrabold text-slate-800">Create New BOA</h2>
            ...
            <form onSubmit={handleCreate} className="p-6 overflow-y-auto space-y-4">
              ...
              <div>
                ...
                <input required type="password" className="w-full rounded-xl border border-slate-200 p-3 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
              </div>
              <div>
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button type="submit" className="flex-1 px-4 py-3 rounded-xl font-bold text-sm text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-200">Create BOA</button>
              </div>
            </form>
          </div>
        </div>
